...
namespace Atents_GameNetWork_04_Server
{
    ...
    {
        static Socket serverSock;   //메인함수가 static 으로 만들어져 있어서 클래스 멤버변수를 static 변수로 선언하는 것(일반변수 불가능)
                                    //static은 인스턴스 단위가 아니라 클래스 단위로 변수가 생성된다
                                    //클래스 변수 생성할 때 new로 생성하는데 그렇게 생성된 객체 각각을 인스턴스라고 한다
                                    //static 멤버가 포함되어 있으면 객체를 생성하지 않는다
        ...
        //public static Queue<User> messageQueue;

        static void Main(string[] args)
        {
            //isInterrupt = false;
            userList = new List<User>();  //자료구조에 유저리스트 보관
            ...
            while (true)    //반복문을 멈출 방법이 없기 때문에 변수 사용
            {
                //예외처리
                try
                {
                    ...
                    //    //    for(int i = 0; i < userList.Count; i++)
                        //    {
                        //        User user = messageQueue.Dequeue();
                        ...
                        //userList[0].userSock.Receive(userList[0].receiveBuffer);        //헐 어러ㅕ워...

                        //userList[i].Receive();
                        //userList[i].ClearSendBuffer();
                        //userList[i].CopyReceiveToSendBuffer();
                        //userList[i].Send();
                        //userList[i].ClearReceiveBuffer();
        ...
        static void NewClient()
        {
            while (true/*!isInterrupt*/)
            {
                ...
                Socket userSock = serverSock.Accept();  //새로 만든 연결에 대한 새 소켓을 할당
