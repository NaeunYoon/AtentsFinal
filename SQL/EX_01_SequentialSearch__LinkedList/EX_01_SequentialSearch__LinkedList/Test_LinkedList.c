...
#pragma endregion
#pragma region 순차탐색 : 앞쪽에서 뒷쪽으로 탐색
    ...
    while (Current !=NULL)
    {
        ...
        if (Current->Data.score == scoreValue)
        {
            ...
            else
            {
                ...
                //이동시킬 노드를 링크에서 제거
                Prev->NextNode = Current->NextNode; //링크에서 빠짐
                //Current를 헤드노드로 만든다
...
#pragma region 전위법 : 한 단계 앞쪽으로 이동
//자기 앞쪽노드의 앞쪽으로 간다.
/*
...
4. 찾는 값을 가진 노드의 앞노드가 찾는 값을 가진 노드의 빈도수보다 작으면 
   찾는 값을 가진 노드를 링크에서 제거한다
